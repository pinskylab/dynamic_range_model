...
# be sure to update file names if copy-pasting this chunk
ggsave(gg_n_p_s_y_hat, filename=here("results","stage_ar_model_adult_dispersal_fit.png"), width=7, height=10)
ggsave(gg_pp_n_p_s_y_hat, filename=here("results","stage_ar_model_adult_dispersal_pp.png"), width=7, height=10)
ggsave(gg_pp_proj_n_p_s_y_hat, filename=here("results","stage_ar_model_adult_dispersal_proj.png"), width=7, height=10)
```
...
# this population model is exactly the same as the one before it, we're just making gA/gY vary with space and time
for (y in 2:years){
  for(p in 1:patches) {
    # recruits
    pop[,1,y] <- round(mean_rec * exp(rnorm(patches,-sigma_r^2/2, sigma_r)))
    ...
    # add reflecting cases for edge patches
    if(p == 1) {
      pop[p,3,y] <- round(pop[p,2,y-1] * (1 - m) * gA[p, y-1]) + # young adults in patch
        round(pop[p,3,y-1] * (1 - m) * (1 - spill)) + # adults in patch, minus mortality and spillover (only to one patch)
        round(pop[p+1,3,y-1] * (1 - m) * spill) # and patch above
    }
...
stage_model_fit <- stan(file = here::here("src","stage_ar_model_T_dep_growth.stan"),
           data = stage_data,
           chains = n_chains,
           warmup = warmups,
           iter = total_iterations,
...
```{r}
# this code is the same for every model at present, except the filenames at the end
mean_recruits_hat <- tidybayes::gather_draws(stage_model_fit, mean_recruits[patch], n = 1000)
...
# be sure to update file names if copy-pasting this chunk
ggsave(gg_n_p_s_y_hat, filename=here("results","stage_ar_model_T_dep_growth_fit.png"), width=7, height=10)
ggsave(gg_pp_n_p_s_y_hat, filename=here("results","stage_ar_model_T_dep_growth_pp.png"), width=7, height=10)
ggsave(gg_pp_proj_n_p_s_y_hat, filename=here("results","stage_ar_model_T_dep_growth_proj.png"), width=7, height=10)
```
...
Using the same model as the block above, just changing the temperature-dependent growth rates we pass to it

```{r stage_ar_model_climate_change}
stage_data <- list(
  m = m,
  gY = gY,
  gA = gA,
...
stage_model_fit <- stan(file = here::here("src","stage_ar_model_T_dep_growth.stan"), # note same model as previous section
           data = stage_data,
           chains = n_chains,
           warmup = warmups,
           iter = total_iterations,
...
# be sure to update file names if copy-pasting this chunk
ggsave(gg_n_p_s_y_hat, filename=here("results","stage_ar_model_climate_change_fit.png"), width=7, height=10)
ggsave(gg_pp_n_p_s_y_hat, filename=here("results","stage_ar_model_climate_change_pp.png"), width=7, height=10)
ggsave(gg_pp_proj_n_p_s_y_hat, filename=here("results","stage_ar_model_climate_change_proj.png"), width=7, height=10)
```
