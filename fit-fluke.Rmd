...
# Dan, can you confirm I did these right? 
mean_age_from_length <- function(length){
  age = log(1-length/Loo)/-k 
  return(age)
}

# NOTE that I think this will produce zero length from zero age which we know is wrong. we need to add in a buffer for size at age 0
...
agedat <- rawdat %>% 
  filter(year>=1972) %>% 
  mutate(age_floor = floor(age), # rounding up so we don't have age zero - should start at age 1
         lat_floor = floor(lat)) %>% 
  rowwise() %>% 
  mutate(
...
length_at_age_key_dat <- generate_length_at_age_key(
  min_age = min(ages), 
  max_age = max(ages), 
...
stage_data <- list(
  ...
  proj_init = pop[,,1],
  ny_proj = years_proj,
  sst = sbt, # change these variable names sometime...
...
stage_model_fit <- stan(file = here::here("src","T_dep_rec_age_str_fluke.stan"), # check that it's the right model!
           data = stage_data,
           chains = 4,
           warmup = warmups,
...
n_p_a_y_hat <-  tidybayes::gather_draws(stage_model_fit, n_p_a_y_hat[patch, age,year], n = 1000)

# pp_n_p_a_y_hat <-  tidybayes::gather_draws(stage_model_fit, pp_n_p_a_y_hat[patch, age,year], n = 1000) # add this back into the model!

pp_proj_n_p_a_y_hat <-  tidybayes::gather_draws(stage_model_fit, pp_proj_n_p_a_y_hat[patch, age,year], n = 1000)
...
sel_at_age <- gather_draws(stage_model_fit, mean_selectivity_at_age[age], n=1000)
...
ggsave(gg_n_p_a_y_hat, filename=here("results","fluke_age_structure_model_predictions.png"), scale=1.5, width=10, height=5)
ggsave(gg_pp_proj_n_p_a_y_hat, filename=here("results","fluke_age_structure_forecast.png"), scale=1.5, width=10, height=5)
