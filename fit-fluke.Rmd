...
agedat <- rawdat %>% 
  ...
  group_by(year, age_floor, lat_floor, haulid) %>% 
  summarise(prep_num_at_age =sum(number_at_length)) %>%  # aggregate numbers by age within each haul
  group_by(year, age_floor, lat_floor) %>% 
  summarise(number_at_age = mean(prep_num_at_age)) %>% # average across all hauls in the band
    ungroup() %>% 
  mutate(number_at_age = as.integer(ceiling(number_at_age))) # convert from density to counts, rounding up so that any positive encounter is at least a 1 (the averages are almost all between 0 and 1 so this  is a big assumption) 
