...
for(p in 1:np){
  for(y in 1:ny){
    ...
    dens[p,y] <- tmp2$mean_dens * patchdat$patch_area_km2[p]
  }
}
...
tmp3 <- n_p_a_y_hat_prop %>% 
  ...
  geom_point(data = n_p_a_y %>% group_by(patch, year) %>% mutate(prop = value / sum(value)) %>% ungroup() %>% filter(patch==3), aes(x=age, y=prop), size = 2,alpha = 0.5) + 
  facet_wrap(~year) 
