data {
  ...
  real gY; // growth rate juveniles -> young adults
  
  real gA; // growth rate young adults -> adults
...
transformed parameters{
  ...
  for (p in 1:np){
    ...
    for (y in 2:ny){
      ...
      n_p_s_y_hat[p,3,y] = n_p_s_y_hat[p,2,y-1] * (1 - m) * gA + n_p_s_y_hat[p,3,y-1] * (1 - m); //AF: got rid of exp(-m) here 
    ...
    //   for(s in 1:ns){
      ...
      for(p in 1:np){
        for(y in 2:ny_proj){
          ...
          tmp_proj[p,3,y] = tmp_proj[p,2,y-1] * (1 - m) * gA + tmp_proj[p,3,y-1] * (1 - m); 
