...
transformed parameters{
  ...
  for(p in 1:np){
    for(a in 1:n_ages){
      if(a==1){
        n_p_a_y_hat[p,a,1] = mean_recruits[p] * exp(raw[1]); // initialize age 0 with mean recruitment in every patch
      }
      else{
  ...
  for (y in 2:ny_train){
    ...
    for(p in 1:np){
      ...
      n_p_a_y_hat[p,1,y] = mean_recruits[p] * exp(rec_dev[y-1]);
      ...
      for(a in 2:(age_at_maturity-1)){
        ...
        c_p_a_y_hat[p,a - 1,y - 1] =  n_p_a_y_hat[p, a-1, y-1] * (z) * (f / (z));
      ...
      for(a in age_at_maturity:n_ages){
        ...
        // if(p==1){
          ...
          // * (1-d) + n_p_a_y_hat[p+1, a-1, y-1] * (1-z) * d;
        // } // close patch 1 case 
        ...
        // } // close if/else for all other patches
          ...
          c_p_a_y_hat[p,n_ages,ny_train] =  n_p_a_y_hat[p, n_ages, ny_train] * (z) * (f / (z));
  ...
  // }
    ...
    for(y in 1:ny_train){
      ...
      // print(c_p_a_y_hat[p,1:n_ages,y]);
  ...
  for(y in 1:ny_train) {
    ...
    for(p in 1:np){
      // if((abund_p_y[p,y]) > 0) {
        ...
        // (n_p_l_y[p,1:n_lbins,y]) ~ multinomial(softmax(to_vector(n_p_l_y_hat[y,p,1:n_lbins])));

        log(abund_p_y[p,y]) ~ normal(log(dens_p_y_hat[p,y] + 1e-6), sigma_obs); 
        
        // log(c_p_y[p,y]) ~ normal(log(c_p_y_hat[p,y] + 1e-6), 0.01);
        
        print("catch is", c_p_y_hat[p,y]);
        
        print("dens is", dens_p_y_hat[p,y]);
        
        // print(abund_p_y[p,y] / (dens_p_y_hat[p,y] * scalar));
