...
model {
  ...
  for(y in 2:ny_train) {
    ...
    raw[y-1] ~ normal(0,sigma_r); // prior on raw process error
      ...
      theta = exp((seen_intercept + seen_sst * sst[y])) / (1 +  exp((seen_intercept + seen_sst * sst[y]))); // calculate detection probability
      
      // if (n_p_s_y[p,n_ages,y] > 0) { // if any stage three are observed
      
      if(sum(n_a_y[sel_100:n_ages,y]) > 0) {
        ...
        // if (min(n_p_a_y[p,1:ns,y]) > 0) {
          ...
          n_a_y[n_ages,y] ~ neg_binomial_2(n_a_y_hat[n_ages,y] * mean_selectivity_at_age[n_ages] + 1e-3, phi_obs); // fit population scale to number of stage three observed
    ...
    //   for(y in 1:ny_proj){
      ...
      for(y in 2:ny_proj){
        ...
        // AF: I just pooled these so that both are added for every age but I'm sure it's wrong somehow 
          ...
          pp_theta = exp((seen_intercept + seen_sst * sst_proj[y])) / (1 +  exp((seen_intercept + seen_sst * sst_proj[y]))); // calculate detection probability
