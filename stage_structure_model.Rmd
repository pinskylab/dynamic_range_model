...
patch_counts <- fluke_patch_stage_year %>% 
  filter(stage == "smalljuv") %>% 
  group_by(patch) %>% 
  summarise(lifetime_n = sum(num_obs)) %>% 
  ungroup() %>% 
  filter(lifetime_n > 0)

fluke_patch_stage_year <- fluke_patch_stage_year %>% 
  filter(patch %in% patch_counts$patch)
...
n_p_s_y <- pop %>% 
  reshape::melt(varnames = c("patch","stage","year")) %>% 
  as_tibble() %>% 
  mutate(stage = case_when(stage == "smalljuv" ~ 1, stage == "largejuv" ~ 2, TRUE ~ 3)) %>% 
  mutate(patch = as.numeric(as.factor(patch)))

n_p_s_y_hat <-  n_p_s_y_hat %>% 
  mutate(year = min(n_p_s_y$year) - 1 + year)

eg_patch <- 2

n_p_s_y_hat %>% 
  filter(patch == eg_patch, year > 2005) %>% 
  ggplot() +
  stat_lineribbon(aes(x = stage, y = .value),.width = c(.9, .6)) +
  geom_point(data = n_p_s_y %>% filter(patch == eg_patch, year > 2005), aes(stage, value + 100, color = "true")) +
...
n_p_s_y_hat %>% 
  ...
  ggplot() +
  geom_point(data = tmp, aes(year, value, color = "observed")) +
  stat_lineribbon(aes(x = year, y = .value),.width = c(.9, .6)) +
  facet_wrap(~patch)
