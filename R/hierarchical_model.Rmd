...
model {
  ...
  # process model
  N[1] ~ dnorm(z0,state.precision) # first time step based on known z0
  for(t in 2:lenT) { # remaining timesteps
    
    #lambda[t] <- N[t-1] * exp(r - r * N[t-1] / K)  # Ricker growth 
    lambda[t] <- N[t-1]
    N[t] ~ dnorm(lambda[t],state.precision) # process error
...
dataOpJags <- list(lenT=nrow(dogfishTest), y=as.double(dogfishTest$estN), z0=mean(dogfishTest$estN[1]), sdp=10, sdo=10)
...
for(i in 1:ncOpJags) {
  ...
  Ninit[1] <- rnorm(1, mean=0, sd=dataOpJags$sdp)
  for(t in 2:dataOpJags$lenT) {
    Ninit[t] <-rnorm(1, mean=Ninit[t-1], sd=dataOpJags$sdp)
  }
