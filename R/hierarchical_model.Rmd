...
model {
  ...
  # priors
  r ~ dnorm(r.hyper.a,r.hyper.b) # intrinsic pop growth rate
  K ~ dunif(K.hyper.a,K.hyper.b) # carrying capacity
  sdo ~ dunif(sdo.hyper.a,sdo.hyper.b) # sd parameter for dnorm obs errors
  sdp ~ dunif(sdp.hyper.a, sdp.hyper.b) # overdispersion parameter for negbin process errors
  
  # sd to precision
  obs.precision <- 1/(sdo*sdo)
  
  # process model
  p[1] <- sdp/(sdp+z0) # convert to dnegbin parameter
  N[1] ~ dnegbin(p[1], sdp) # first time step based on known z0
  for(t in 2:lenT) { # remaining timesteps
    
    lambda[t] <- N[t-1] * exp(r - r * N[t-1] / K)  # Ricker growth 
    p[t] <- sdp/(sdp+lambda[t]) # convert to dnegbin parameter
    N[t] ~ dnegbin(p[t], sdp) # process error
...
This takes a few minutes to run on my laptop. Slowed down a little with K~runif(1, 100000). Slowed down a lot with dnegbin process errors.
...
for(i in 1:ncJags) {
  ...
  sdpinit <- runif(1, min=dataJags$sdp.hyper.a,
                   max=dataJags$sdp.hyper.b)
  ...
  initvalJags[[i]] <- list(sdo = sdoinit, sdp=sdpinit, r=rinit, K=Kinit, N=Ninit)
}
