...
# Summarize length data into length classes CURRENTLY NEUS ONLY
...
```{r summer flounder}
# exploring summer flounder data dist. for stats class

fluke_neus_len <- neus_len %>% filter(spp=="Paralichthys dentatus")

fluke_lm <- 28 # FOR EXPLORATORY PURPOSES ONLY! FROM FISHBASE--REVISIT THIS
fluke_ljuv <- min(fluke_neus_len$LENGTH) + (fluke_lm - min(fluke_neus_len$LENGTH))/2

dat_exploded_fluke <- dat.exploded %>%
  filter(spp=="Paralichthys dentatus",
         region %in% regOfInt
  ) 
...
fluke_len_zeros <- expand_grid(haulid=unique(dat_exploded_filter$haulid),  lengthclass=unique(fluke_neus_len_agg$lengthclass),
                                 spp="Paralichthys dentatus") %>%  
  left_join(fluke_neus_len_agg, by=c("spp","haulid","lengthclass")) %>% 
  left_join(dat_exploded_fluke, by=c("spp","haulid")) %>% 
  left_join(btemp, by="haulid") %>%
  select(-wtcpue) %>% 
  mutate(numlengthclass = replace_na(numlengthclass, 0)) 

fluke_len_zeros %>% 
  filter(year==1975, lat < 41, lat >= 40, lengthclass=="adult") %>% 
  ggplot() +
  geom_histogram(aes(x=numlengthclass))

fluke_len_zeros %>% 
  filter(year==1975, lat < 41, lat >= 40, lengthclass=="adult", numlengthclass>0) %>% 
  ggplot() +
  geom_histogram(aes(x=numlengthclass))

fluke_len_zeros %>% 
  filter(year==2009, lat < 41, lat >= 40, lengthclass=="adult", numlengthclass>0) %>% 
  ggplot() +
  geom_histogram(aes(x=numlengthclass))

fluke_len_zeros %>% 
  filter(year==1999, lat < 41, lat >= 40, lengthclass=="adult", numlengthclass>0) %>% 
  ggplot() +
  geom_histogram(aes(x=numlengthclass))
