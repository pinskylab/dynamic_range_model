...
fluke_neus_fall <- fluke_neus_len %>% 
  ...
  filter(region=="Northeast US Fall") %>% 
  mutate(numlen_correct = ifelse(LENGTH<30, NUMLEN*(1/0.72), ifelse(LENGTH<40, NUMLEN*(1/0.80), NUMLEN)))
...
fluke_neus_len_agg <- bind_rows(fluke_neus_fall, fluke_neus_spring) %>%
  mutate(lengthclass = ifelse(LENGTH <= fluke_ljuv, "smalljuv",
                              ifelse(LENGTH < fluke_lm, "largejuv", "adult"))) %>% 
  group_by(haulid, spp, lengthclass) %>% 
  summarise(numlengthclass = sum(numlen_correct))  
...
squid_lm <- 20 # FOR EXPLORATORY PURPOSES ONLY! based on a best guess from this sentence in the FAO page: "Summer hatchlings achieve a mantle length of about 18 or 19 cm after 1 year (females are slightly larger than males) and are ready to spawn in winter at an age of about 1 ½ years (Mesnil, 1977)." http://www.fao.org/fishery/species/2720/en
