...
* make r, lambda0, gamma_rate0, and gamma_shape0 spatially and temporally varying (currently estimated as one parameter)
...
y_array <- reshape2::acast(fluke_lat_time, lat_round~year, value.var="num_obs") # could also do this with pivot_wider
...
z0 <- fluke_lat_time %>% filter(year == min(year)) %>% arrange(lat_round) %>% pull(num_obs)

# starting values from real data 
dataStan1 <- list(len_t=length(unique(fluke_lat_time$year)),
                  len_i=length(unique(fluke_lat_time$lat_round)),
