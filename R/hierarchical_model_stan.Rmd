...
real<lower=0, upper=K_hyper_upper> phi_obs_hyper_mean;
real<lower=0> phi_obs_hyper_sd;
...
real<lower=0, upper=K_hyper_upper> phi_obs;
...
phi_obs ~ normal(phi_obs_hyper_mean, phi_obs_hyper_sd);
...
initStan1 <- list()
for(i in 1:n_chains) {
  phi_obs_hyper_mean_init <- rpois(1, dataStan1$z0) # not sure if this is the right way to think about this parameter now that it isn't sigma_obs
  phi_obs_hyper_sd_init <-rnorm(1, mean=100, sd=10)
  ...
  phiinit <- rnorm(1, mean=phi_obs_hyper_mean_init,
                   sd=phi_obs_hyper_sd_init)
  ...
  initStan1[[i]] <- list(
    ...
    phi_obs_hyper_mean=phi_obs_hyper_mean_init,  
    phi_obs_hyper_sd=phi_obs_hyper_sd_init,
