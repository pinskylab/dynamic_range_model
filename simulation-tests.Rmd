...
Topt_hat <- tidybayes::gather_draws(stage_model_fit, Topt, n=1000)
...
# Taking parameters as known 

Because that didn't work very well without very tight priors and/or continuing to treat parameters as known, let's change the process model so that recruitment, not growth, is temperature-dependent. I'm also making the process error constant across patches, and the mean recruits constant over all space and time. Take Two:

## Fit a model with temperature-dependent recruitment (m, g fixed)
...
sigma_r <- 0.1 # turned this down 

# upgrading mean_rec to vary with time as well as patch -- otherwise the temperature-dependent recruitment is too heavily influenced by the values it draws 
#mean_rec <- array(NA, dim = c(patches, years))

# for(y in 1:years){
#   mean_rec[,y] <- rlnorm(patches, log(50),0.5) 
# } # made this mean bigger to compensate for depressing estimated recruitment with T-dependence, and turned down the variance, because otherwise the plots didn't look like nice temperature-dependent pop dy 

# let's try just one mean_rec
mean_rec = rlnorm(1, log(50),0.5) 

n0 <- mean_rec * matrix(rep(exp(-m * (0:(stages - 1))),patches), nrow = patches, ncol = stages, byrow = TRUE)

width <- 4 # sensitivity to temperature variation 

sstopt <- 18 # temp at which recruitment is maximized

T_dep <- function(sst) {exp(-0.5 * ((sst - sstopt)/width)^2)}
plot(seq(1, 100, 1), T_dep(seq(1, 100, 1))) # check how this works

spill <- 0.1

sst <- array(NA, dim = c(patches, years))
T_adjust <- array(NA, dim = c(patches, years))
...
for(p in 1:patches){
  for(y in 1:years){
    ...
    T_adjust[p, y] <- T_dep(sst[p, y]) # calculate array of recruitment tuning parameters
  }
}
...
rec_dev <- array(NA, dim=years)

for (y in 2:years){
  rec_dev[y] <- exp(rnorm(1,-sigma_r^2/2, sigma_r))
  for(p in 1:patches) {
    # recruits
    pop[p,1,y] <- round(mean_rec * rec_dev[y] * T_adjust[p,y]) # adding temperature dependence. 
    
    # young adults / large juvs
    pop[p,2,y] <- round(pop[p,1,y-1] * (1 - m) * g) + # recruits, less mortality, plus growth into this stage
      round(pop[p,2,y-1] * (1 - m) * (1 - g)) # this life stage last year less individuals that died or grew out 
    
    # adults
    
    # add reflecting cases for edge patches
    if(p == 1) {
      pop[p,3,y] <- round(pop[p,2,y-1] * (1 - m) * g) + # growth from young adults in patch
        round(pop[p,3,y-1] * (1 - m) * (1 - spill)) + # adults in patch, minus mortality and spillover (only to one patch)
        round(pop[p+1,3,y-1] * (1 - m) * spill) # and patch above
    }
...
stage_data <- list(
  ...
  proj_init = pop[,,years_train],
  ny_proj = years_proj, 
  m = 0.2,  # these are very arbitrary
  g = 0.5
)
...
mean_recruits_hat <- tidybayes::gather_draws(stage_model_fit, mean_recruits, n = 1000) # used to be mean_recruits[patch]
...
ggsave(gg_n_p_s_y_hat, filename=here("results","T_dep_rec_fit.png"), width=7, height=10)
ggsave(gg_pp_n_p_s_y_hat, filename=here("results","T_dep_rec_pp.png"), width=7, height=10)
ggsave(gg_pp_proj_n_p_s_y_hat, filename=here("results","T_dep_rec_proj.png"), width=7, height=10)
```
