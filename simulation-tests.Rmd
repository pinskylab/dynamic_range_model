...
mean_rec <- rlnorm(patches, log(50),0.5) # made this mean bigger to compensate for depressing estimated recruitment with T-dependence, and turned down the variance, because otherwise the plots didn't look like nice temperature-dependent pop dy 

n0 <- mean_rec * matrix(rep(exp(-m * (0:(stages - 1))),patches), nrow = patches, ncol = stages, byrow = TRUE)

width <- 6 # sensitivity to temperature variation 
...
for (y in 2:years){
  for(p in 1:patches) {
    # recruits
    pop[,1,y] <- round(mean_rec * exp(rnorm(patches,-sigma_r^2/2, sigma_r)) * T_adjust[p,y]) # adding temperature dependence. 
