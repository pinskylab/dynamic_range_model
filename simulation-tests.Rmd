...
Because that didn't work very well without very tight priors and/or continuing to treat parameters as known, let's change the process model so that recruitment, not growth, is temperature-dependent. Take Two:
...
n0 <- mean_rec[,1] * matrix(rep(exp(-m * (0:(stages - 1))),patches), nrow = patches, ncol = stages, byrow = TRUE)
...
for (y in 2:years){
  for(p in 1:patches) {
    # recruits
    pop[,1,y] <- round(mean_rec[p,y] * exp(rnorm(patches,-sigma_r^2/2, sigma_r)) * T_adjust[p,y]) # adding temperature dependence. 
